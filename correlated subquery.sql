...
SELECT
    ...
    ,(  SELECT MIN(PatientId) FROM PatientStay WHERE PatientId > ps.PatientId) - (  SELECT MAX(PatientId) FROM PatientStay WHERE PatientId < ps.PatientId) AS Gap
FROM
    PatientStay ps
ORDER BY
    Gap DESC
...
SELECT   abc.PatientId,
         ...
         (abc.NextBookedPatientId - PreviouslyBookedPatientId) / 2 AS gap
